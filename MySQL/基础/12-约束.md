...
> 总结：能约束多个列的，且能单独定义的多列约束，和不能单独定义的，只能约束一个列的单列约束。
...
CREATE TABLE USER(
 ...
 PASSWORD VARCHAR(16),
 -- 使用表级约束语法
 CONSTRAINT uk_name_pwd UNIQUE(NAME,PASSWORD) # uk_name_pwd 是 自己起的唯一性约束的名字
);
```
