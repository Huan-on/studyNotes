...
```java
    ...
    public  void testConnection5() throws Exception {
        ...
        //使用 ClassLoader 类自带的系统的 getSystemClassLoader 类加载器，来获取文件
...
>
> 这一条语句，本来是一个 且的关系，也就是 and 的关系，但是经过这样一搞，就变成了 或的关系，也就是 or 的关系，而在 or 中，又存在一个恒成立的表达式 '1' = '1';，自然会出问题。
...
> 查询详解：
>
> 1. 获取连接，得到 Connerction 对象
> 2. 编写预编译的 SQL 语句
> 3. 填充占位符(可以使用循环，但是需要注意，占位符的起始值为 1 ，数组的起始值为 0)
> 4. 执行查询语句，并使用 ResultSet 对象获取查询到的结果
> 5. 获取结果集的总列数，需要一个 元数据，该 元数据使用 ResultSet 对象获取，方法为 getMetaData()，返回的对象为 ResultSetMetaData 元数据对象，使用 ResultSetMetaData 元数据对象获取 结果集的总字段数(列数)
> 6. 获取总列数的方法为 getColumnCount()，返回一个 int ，表示列的数目
> 7. 使用迭代器，迭代器对标到每一行，取出每一行的数据，并判断下一行是否存在数据，存在就继续下移，不存在就停止下移。该迭代器是由 ResultSet 对象提供。(可以这样理解，元数据表示具体某一行的数据，而 ResultSet 则表示许多行数据所组成的一个结果集)
> 8. 循环取出每一个列的具体的值，使用 ResultSet 对象提供的 getObject(列数int) 方法，表示获取当前 列数int 所在列的具体值。但是需要注意，数据库列的起始值是 1，如果循环条件是从 0 开始，则需要 +1，
> 9. 将取出的数据循环封装到对象中，使用 空参对象 + 反射 的方式，将每一个值封装到该对象中去，通过 ORM 思想可以得到，一个字段必然对应一个相同名字的属性，所以需要先拿到每一个列的列名，一个列名对应一个属性，通过列名和反射爆破，获取到所需要的类的属性对象：`Field field = clazz.getDeclaredField(columnName);`，然后通过操作其属性对象，进行赋值操作，就可以将数据完整的封装到一个对象中了。
...
> 预编译就代表着，这条 sql 语句不会将 且的关系 再次搞错成为 或的关系，即
>
> SELECT user, password FROM user_table WHERE user='a' OR 1 = ' AND password = ' OR '1' = '1'
>
> 这条非法的 sql 语句所存在的 sql 注入问题就失效了。
>
> 预编译代表着提前进行过 sql 校验，也可以理解为 提前运行过，这样就代表着，sql 语句所表示的语义不会被曲解，且的关系 就是 end 的关系，or 的关系就是 or 的关系，不存在 and 的关系被曲解成 or 的关系的情况。
...
#### 3.3.6 使用PreparedStatement实现返回多条结果的查询操作
...
```java
   String sql1 = "select id,name,email,birth from customers where id < ?";
        List<customers> list = getForList(customers.class, sql1, 13);
		//list方法，一个很方便的遍历 list 集合的方法，forEach(System.out::println)
...
> 注意：使用了 getColumnLabel() 方法后，即使该列没有起别名，则会直接获取到当前的列名。其实，也可以理解为，getColumnLabel() 就是获取显示出来的列名，列名显示的时候是什么，那么获取到的列名就是什么。
...
#### 6.3.4 在 Java 代码中设置数据库的隔离级别

```java
	Connection conn;//这里需要获取数据库连接，得到连接对象
	conn.setTransactionIsolation(Connection./*具体的隔离级别*/)
```

> setTransactionIsolation 方法能在 java 代码中设置**当前连接**的隔离级别
>
> 注意：是**当前连接**，也就是当前会话的，**不能跨连接和跨重启**。
...
public abstract class BaseDao<T> {
    ...
    //上面的方法也可以写在代码块中，就是普通代码块，静态代码块调用不了动态的方法，实现效果与在构造器中是一样的。
...
> 总结：一个数据库表往往对应着3个类和1个基础父类(抽象)
>
> ORM 思想代表着一个表需要对应着一个类
>
> MVC 思想代表着一个类需要对应着一个接口和一个该接口的实现类，该实现类，需要继承一个有着通用的增删改查操作的抽象父类，通过继承获得抽象父类的增删改查方法，然后实现接口的规范，在具体实现中，可以随时调用从抽象父类那获得的增删改查
>
> 自我感悟：代码分层，每一个模块都有着自己的功能，代码健壮性和复用性以及可维护性都很强，这就是 Java 的魅力啊。
